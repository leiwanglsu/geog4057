import os
def readSingleYSI(filename,new_pos_dict):
    ...
    while True:
        ...
        if len(nstr) == 0:
            ...
            if(strs[0]=="SMI"):
                ...
                ysi_dict[time_tag] = data
    for ysi in ysi_dict.items():
        search_key = ysi[0]
        key = min(pos_dict.keys(), key = lambda key: abs(key-search_key))
        new_pos_dict[key] = [search_key,pos_dict[key],ysi[1]]
    return new_pos_dict
def readYSI(pathname):
    pos_dict = {}
    for filename in os.listdir(pathname):
        f = os.path.join(pathname, filename)
        file_extension = os.path.splitext(f)[1]
        if(file_extension == ".RAW"):
            pos_dict = readSingleYSI(f,pos_dict)
    return pos_dict
